...
class SecurityPriceHistory < ActiveRecord::Base
  ...
  URL_SH_AND_SZ = 'http://123.57.249.182:8080/URL/Api/ApiInterface.ashx?apikey=3960f9e2bdf438e80d04e7637d2e6f7a&flag=gethData&'

  def self.record_history(security_info, response)
    if response.code == 200
      result = JSON.parse(response.body)
      if result['rows'].size > 0
        security_price_history = SecurityPriceHistory.new(result['rows'][0].except('code'))
